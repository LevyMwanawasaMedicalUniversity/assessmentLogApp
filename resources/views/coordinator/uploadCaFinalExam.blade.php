    ...
    <main id="main" class="main">
        <div class="pagetitle">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                ...
                <b>{{ $results->CourseDescription }} - {{$results->CourseName}} : {{$delivery}} @isset($hasComponents) in {{$hasComponents}} @endisset FINAL EXAMINATION</b></span>
            </h2>
            ...
            <div class="row">
                <div class="col-lg-12">
                        <div class="card"><div class="card-header {{ $delivery == 'Fulltime' ? 'bg-primary' : ($delivery == 'Distance' ? 'bg-success' : '') }} text-white">                        
                            <h5><b>Upload ( {{$delivery}} Education ) @isset($hasComponents) in {{$hasComponents}} @endisset FINAL EXAMINATION</b></h5>
                        </div>
                        <div class="card-body">
                            <!-- Vertical Form -->
                            <div class="container px-4">
                                <div class="row pt-3">
                                    <div class="col-12">
                                        @include('coordinator.components.excelSheetormFinalExam')
                                    </div>
                                </div>
                            </div>
                        </div>
