<div class="bg-white shadow rounded-lg p-6 border border-gray-300">
    ...
    <!-- File Upload Form -->    
        <form action="{{ route('coordinator.importFinalExamFromExcelSheet') }}" method="POST" enctype="multipart/form-data" class="p-4 bg-light border rounded shadow-sm">
            @csrf

            <!-- Form Header -->
            <div class="mb-4">
                <h3 class="font-weight-bold {{ $delivery == 'Fulltime' ? 'text-primary' : ($delivery == 'Distance' ? 'text-success' : '') }}">
                    Import {{$delivery}} Exam Results
                </h3>
