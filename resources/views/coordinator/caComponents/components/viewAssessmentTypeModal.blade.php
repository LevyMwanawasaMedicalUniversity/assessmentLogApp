<div class="container">
    <div class="modal fade" id="viewCourseModal{{ $result->course_components_id }}{{ $result->delivery_mode }}{{$result->study_id}}" tabindex="-1" role="dialog" aria-labelledby="viewCourseModalLabel" aria-hidden="true">
                            ...
                            @if($courseAssessmenetTypes->count() > 0)
                                @if( $totalMarks == 40)
                                    @foreach ($courseAssessmenetTypes as $courseAssessmenetType )
                                        <form method="GET" action="{{ route('coordinator.viewAllCaInCourse', ['statusId' => encrypt($courseAssessmenetType->id), 'courseIdValue' => encrypt($result->course_id),'basicInformationId' => encrypt($basicInformationId),'delivery'=>encrypt($result->delivery_mode)]) }}">
