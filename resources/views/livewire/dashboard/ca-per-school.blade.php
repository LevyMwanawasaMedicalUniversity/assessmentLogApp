<div class="card" x-data="caPerSchoolStore()">
    ...
    <div class="card-body position-relative">
        ...
        <div x-show="data.length > 0" x-cloak>
            ...
            <div id="caPerSchoolChart" style="height: 400px;" class="echart mb-4"></div>
        </div>
    </div>
        ...
        return {
            ...
            init() {
                ...
                window.addEventListener('resize', () => {
                    if (window.caPerSchoolChart) {
                        window.caPerSchoolChart.resize();
                    }
                });
            },
                ...
                fetch('{{ route('api.dashboard.ca-per-school') }}')
                    ...
                    .then(data => {
                        if (data.status === 'success') {
                            // Extract the data
                            const schoolsData = data.caPerSchool || [];
                            
                            if (schoolsData && schoolsData.length > 0) {
                                this.data = schoolsData;
            ...
            renderChart() {
                ...
                try {
                    ...
                    const schoolNames = this.data.map(school => school.school_name);
                    const coursesWithCA = this.data.map(school => parseInt(school.courses_with_ca) || 0);
                    const totalCourses = this.data.map(school => parseInt(school.total_courses) || 0);
                    ...
                    if (window.caPerSchoolChart && typeof window.caPerSchoolChart.dispose === 'function') {
                        window.caPerSchoolChart.dispose();
                    }
                    
                    // Initialize chart
                    window.caPerSchoolChart = echarts.init(chartDom);
                    ...
                    // Apply the options
                    window.caPerSchoolChart.setOption(option);
                } catch (error) {
